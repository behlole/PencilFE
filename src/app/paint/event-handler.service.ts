...
export class EventHandlerService {
  ...
  canvaData:BehaviorSubject<any>=new BehaviorSubject("");
  isSubcriing :BehaviorSubject<boolean>=new BehaviorSubject(false);
  image:any
  ...
  addImageOnCanvas(ImageBase64:any) {
...
getCanvaData(){
  this.canvaData.next(this.canvas.toDataURL());
}
AddDataOnCanvaUser(Data){
  this.imageDataUrl=Data;
  this.addBGImageSrcToCanvas();
}
ChangeValur(isSubsribe:boolean){
  this.isSubcriing.next(isSubsribe);
}
}
